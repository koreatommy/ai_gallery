...
- **usePagination 훅**: `useInfiniteScroll`을 `usePagination`으로 리팩토링

### 관리자 기능 강화
- **이미지 카테고리 변경**: 관리자 대시보드에서 개별 이미지의 카테고리 변경 가능
...
- **낙관적 업데이트**: 클릭 시 즉시 UI 반영 후 서버 동기화
- **에러 처리 강화**: 실패 시 자동 롤백으로 일관성 유지
- **중복 방지**: 로딩 상태로 중복 클릭 방지
- **실시간 동기화**: 서버 응답에 따른 상태 조정
- **향상된 UX**: 명확한 성공/실패 메시지, 로딩 상태 표시

### 기술적 개선사항
- **새로운 파일**: `src/lib/userManager.ts` - 사용자 관리 시스템
